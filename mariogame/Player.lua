...
function Player:init(map)
    ...
    self.behaviors = {
        ...
        ['walking'] = function(dt)
            ...
            left1 = self:checkLeftCollisionFlag()
            right1 = self:checkRightCollisionFlag()

            if left1 or right1 then
                love.graphics.setColor(1,1,1,1)
                love.graphics.print("YOU WIN", 600, 360)
            end
        ...
        ['jumping'] = function(dt)
            ...
            self:checkRightCollision()
            self:checkLeftCollision()
            left2 = self:checkLeftCollisionFlag()
            right2 = self:checkRightCollisionFlag()

            if left2 or right2 then
                love.graphics.setColor(1,1,1,1)
                love.graphics.print("YOU WIN", 600, 360)
            end
        end
...
function Player:checkLeftCollisionFlag()
    if self.dx < 0 then
        -- check if there's a tile directly beneath us
        if self.map:flagpoleCollides(self.map:tileAt(self.x - 1, self.y)) or
            self.map:flagpoleCollides(self.map:tileAt(self.x - 1, self.y + self.height - 1)) then
...
function Player:checkRightCollisionFlag()
    if self.dx > 0 then
        -- check if there's a tile directly beneath us
        if self.map:flagpoleCollides(self.map:tileAt(self.x + self.width, self.y)) or
            self.map:flagpoleCollides(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) then
